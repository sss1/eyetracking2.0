...
def preprocess_all(eyetrack, target, distractors, labels = None, behavioral = None, trial_discard_threshold = 0.5, subject_discard_threshold = 0.5):
  is_labeled = labels != None
  has_behavioral = behavioral != None
  ...
  if np.mean([trial_data is None for trial_data in eyetrack]) > subject_discard_threshold:
    ...
    if has_behavioral:
      behavioral = filter_None(behavioral)

  if is_labeled:
    return eyetrack, target, distractors, labels
  if has_behavioral:
    return eyetrack, target, distractors, behavioral
